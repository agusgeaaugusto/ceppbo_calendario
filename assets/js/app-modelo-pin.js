...
if (item.imgData) {
  ...
  // Si no hay imagen, mostrar un bloque de color
  img.remove(); // no ponemos <img>
  const colorBox = document.createElement('div');
  colorBox.className = 'thumb color-thumb';
  colorBox.style.background = item.color || '#444'; // fallback gris si no elige color
  card.append(colorBox, content);
  cards.appendChild(card);
  continue; // saltamos para que no duplique el append
}
